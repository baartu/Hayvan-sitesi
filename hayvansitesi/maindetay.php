...
if (isset($_POST["buton"])) {
    ...
    $icerik = $_POST['icerik'];
    $urun_id = $_GET["id"]; // Detaylar sayfasında gösterilen ürün ID'sini alın

    $check_tc_sql = "SELECT user_eposta FROM register WHERE user_eposta = '$email'";
    $result = $conn->query($check_tc_sql);

    if ($result->num_rows > 0) {
        $sql = "INSERT INTO yorum (yorum_eposta, yorum_icerik, sss_id) VALUES ('$email', '$icerik', $urun_id)";
        // Sorguyu çalıştır
        if ($conn->query($sql) == TRUE) {
            echo '<div class="alert alert-success text-center" role="alert">
                     Yorum Başarı İle Eklendi.
                     </div>';
            echo "<meta http-equiv='refresh' content='3;url=maindetay.php?id=$urun_id'>";
        }
        // Veritabanı bağlantısını kapat
        $conn->close();
    } else {
        echo ' <div class="alert alert-danger text-center" role="alert">
 Kayıtlı Kullanıcı Bulunamadı !!! Kayıt Ekranına Yönlendiriliyosunuz.
</div>';
...
<body>
    ...
    <div class=" w-100 h-25">
        ...
        if ($_GET["id"] != null) {
            $urunid = $_GET["id"];
            $sql = "SELECT * FROM sss where sss_id =$urunid";
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<img src="' . $row["user_dosya"] . '" class="img-fluid rounded-start" alt="...">';
                echo '</div>';
                ...
                echo '<p class="card-text"><small class="text-body-secondary">' . $row["user_eposta"] . '</small></p>';
                echo '  </div>';
                echo ' </div>';
            }
            echo '</div>';
    ...
    <div class="container-fluid py-5 border">
        <div class="row justify-content-center align-items-center ">
            <div class="col-12 col-sm-8 col-lg-6">
                ...
                include 'baglanti.php';
                $yorum_sql = "SELECT * FROM yorum WHERE sss_id = $urunid";
                ...
                if ($yorum_result->num_rows > 0) {
                    while ($yorum = $yorum_result->fetch_assoc()) {
                        ...
                        echo '<div class="card-body">';
                        echo '<h5 class="card-title">' . $yorum["yorum_icerik"] . '</h5>';
                        echo '<small class="card-text text-secondary">' . $yorum["yorum_eposta"] . '</small> <br>';
                        echo '<small class="card-text text-secondary ">' . $yorum["yorum_saat"] . '</small>';
                        echo '</div>';
                        echo '</div>';
                    }
                } else {
